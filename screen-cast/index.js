...
var iceServers = null;
var	server = "https://mc.sdq.st/janus";
var janus = null;
var screentest = null;
			...
			janus = new Janus(
				{
                  ...
                  if(urlParams.has("sid")) {
                    $('#roomid').val(urlParams.get("sid"));
                    setTimeout(()=>$('#join').click(), 1000);
                  }
									// $('#desc').focus();
								},
								...
								onmessage: async function(msg, jsep) {
									...
									if(event) {
										if(event === "joined") {
											myid = msg["id"];
											$('#session').html(`<a href="javascript:void(0)" style="color: white;" onclick="copyText('${room}')">${room} (click to copy)</a>`);
                      ...
                      // urlParams
                          ...
                          const stream = await navigator.mediaDevices.getDisplayMedia({ 
                            video: { frameRate: 30, height: 720, width: 1280 }, 
				                    audio: { channelCount: 2, echoCancellation:false, autoGainControl:false, noiseSuppression:false },
                          }); 
                          const tracks = []; 
                          for (let track of stream.getVideoTracks()) {
                            tracks.push({ type: 'screen', capture: track, recv: false }); 
                          } 
                          
                          for (let track of stream.getAudioTracks()) 
                          {
                            tracks.push({ type: 'audio', capture: track, recv: false });
                          } 
                                ...
                                customizeSdp: function(jsep) {
                                  jsep.sdp = jsep.sdp.replace('useinbandfec=1', 'useinbandfec=1;stereo=1;maxaveragebitrate=128000');
                                },
															success: function(jsep) {
																Janus.debug("Got publisher SDP!", jsep);
																let publish = { request: "configure", audio: true, video: true };
												...
												if($('#screencapture .no-video-container').length === 0) {
													$('#screencapture').append(
														'<div class="no-video-container">' +
															...
															'<span class="no-video-text">No stream available</span>' +
														'</div>');
                    ...
                    if(urlParams.has("sid")) {
                      makeFullScreen('screenvideo' + trackId);
                    }
...
function shareScreen() {
	...
	let create = {
		request: "create",
		description: desc,
		bitrate: 1024000, //500000 
		publishers: 1
	};
  if(urlParams.has("room")) {
    create.room = Number(urlParams.get("room"));
    if(isNaN(create.room)){
      delete create.room;
    }
  }
	screentest.send({ message: create, success: function(result) {
		if(result["error"]) {
			// bootbox.alert(
      console.log("Couldn't create room: " + result["error"] + ", destroying...");
      if(result["error"].indexOf("already exists") > -1) {
        const destroy = {
          "request" : "destroy",
          "room" : create.room,
        }
        screentest.send({ message: destroy, success: function(result) {
          shareScreen();
        }});
      }
			return;
	...
	janus.attach(
		{
          ...
          if(urlParams.has("sid")) {
            makeFullScreen('screenvideo' + mid);
          }
					$('#screenvideo' + mid).get(0).volume = 0;
					Janus.attachMediaStream($('#screenvideo' + mid).get(0), stream);
					$('#screenvideo' + mid).get(0).play();
